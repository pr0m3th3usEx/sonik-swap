...
use snk_core::{commands::create_credentials_user::{CreateCredentialsUserCommand, CreateCredentialsUserCommandError}, contracts::{providers::{password_provider::PasswordProvider, user_id_provider::UserIdProvider}, repositories::user_repository::UserRepository}};
  ...
  PassswordProv: PasswordProvider,
{
  CreateCredentialsUserCommand::new(payload.email, payload.password)
      .execute(&state.user_repo, &state.user_id_provider, &state.password_provider)
      .await?;
